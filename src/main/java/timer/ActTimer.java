...
public class ActTimer {
	...
	public boolean start(int threadCount) {
		synchronized (kMutex) {
			...
			ScheduledThreadPoolExecutor exe = new ScheduledThreadPoolExecutor(
					threadCount, new ThreadFactory() {
						...
						public Thread newThread(Runnable r) {
							return new Thread(r, "ActTimer-" + name + "-"
	...
	public ScheduledFuture<?> register(long periodMS, long delay, Runnable r,
			IActor target, String name) {
		synchronized (kMutex) {
			return executor.scheduleAtFixedRate(new RepeatCountRunnable(r,
					target, name), delay, periodMS, TimeUnit.MILLISECONDS);
		}
	}

	public ScheduledFuture<?> register(long periodMS, Runnable r,
			IActor target, String name) {
		synchronized (kMutex) {
			return executor.scheduleAtFixedRate(new RepeatCountRunnable(r,
					target, name), 0, periodMS, TimeUnit.MILLISECONDS);
		}
	}

	public ScheduledFuture<?> register(long periodMS, long delay,
			int repeatLimit, Runnable r, IActor target, String name) {
		synchronized (kMutex) {
			RepeatCountRunnable runnable = new RepeatCountRunnable(r, target,
					repeatLimit, name);
	...
	public ScheduledFuture<?> schedule(long delay, Runnable r, IActor target,
			String name) {
		synchronized (kMutex) {
			return executor.schedule(new RepeatCountRunnable(r, target, name),
					delay, TimeUnit.MILLISECONDS);
		}
	}

	public ScheduledFuture<?> scheduleAtTimestamp(long timestamp, Runnable r,
			IActor target, String name) {
	...
	private static class RepeatCountRunnable implements Runnable {
		...
		public RepeatCountRunnable(Runnable r, IActor target, int repeatLimit,
				String name) {
			if (target == null) {
	...
	public String getActorStatus() {
		...
		ret.append(executor.getQueue().size());
		return ret.toString();
	}
